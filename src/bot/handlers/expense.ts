...
export function register(bot: Bot) {
  bot.command("xarajat", async (ctx) => {
    ...
    xarajatOrnat(ctx.from!.id, { qadam: "summa" });
    await ctx.reply(
  ...
  bot.command("otkaz", async (ctx) => {
    if (ctx.from) xarajatTozala(ctx.from.id);
    await ctx.reply("Bekor qilindi.");
  });

  bot.on("message:text", async (ctx, next) => {
    if (ctx.chat.type !== "private" || !ctx.from) return next();
    const holat = xarajatHolati(ctx.from.id);
    ...
    if (holat.qadam === "summa") {
      ...
      xarajatOrnat(ctx.from.id, { qadam: "izoh", summa });
      return ctx.reply(`✅ ${pul(summa)}\n\nNima olib keldingiz? Qisqacha yozing.`);
    }

    if (holat.qadam === "izoh") {
      xarajatOrnat(ctx.from.id, {
        qadam: "rasm",
        summa: holat.summa,
  ...
  async function saqla(ctx: any, summa: number, izoh: string, photoId: string | null) {
    const u = await kim(ctx.from.id);
    if (!u) return;
    xarajatTozala(ctx.from.id);
